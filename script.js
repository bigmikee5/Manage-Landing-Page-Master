...
function addCard(initCards, isEmpty = true, cards) {
  for (let i = 0; i < initCards.length - 1; i++) {
    ...
    if (isEmpty) {
      isEmpty = false;
      return;
    } else if (!initCards[i][1]) cards[0].parentNode.appendChild(newChild);
  }
}
...
function moveCards(elements, reset = false) {
  for (let i = 0; i < elements.length; i++) {
    ...
    if (i === 3) elements[3].classList.add("-active");
...
function auto() {
  ...
  setTimeout(
    function cardCarussel(cards, time) {
      go(cards, delay * 0.9);
...
function initCardsSelectionCircle(elements) {
  ...
  for (let i = 0; i < elements.length; i++) {
    ...
    } else cardCircle.classList.add("circle", "_slider");
...
function showCardSelection(elements, init = false) {
  ...
  if (window.outerWidth > 1190) slider.style.display = "none";
  else if (init) {
